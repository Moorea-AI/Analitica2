...
Original file is located at
    https://colab.research.google.com/drive/1nBrh_eaRJtEse_xs4Q2_wLaxFNxn-wLr
...
#LLamado a la base de datos ALGEBRA
ALGEBRA = pd.read_csv('https://www.4minds.solutions/Analitica2/algebra_vectorial.csv', sep=';',  low_memory=False) #Base de datos de Algebra Vectorial
#Revisamos su contenido
ALGEBRA

"""# **Análisis a las base de datos de Algebra y Cálculo**
#### Hacemos el análisis exploratorio de ambas bases de datos, empezando por cálculo y luego replicando lo mismo en Algebra
...
ALGEBRA.drop(ALGEBRA.columns[0], axis=1, inplace=True)

"""Se decide eliminar la primera columna "Unnamed" ya que es un conteo y no es relevante en la base de datos"""

display(ALGEBRA[ALGEBRA.duplicated(keep='last')])
display(ALGEBRA[ALGEBRA.duplicated(keep='first')])

"""Observamos que hay 2 pares de duplicados en la BD

La fila 0 y la 649
La fila 424 y la 650
Se toma la decisión de dejar los duplicados, ya que se asume que son situaciones reales y que realmente 2 estudiantes pueden tener esas mismas caracteristicas.
...
ALGEBRA.describe(include='object').transpose()

"""De este *describe* de las variables categoricas, podemos observar que:
* Las variables *soporte_edu_extra, monitores y postgrado*, tiene una categoria dominante con registros por encima de 580.
...
cat_cols = [col for col in ALGEBRA.columns if ALGEBRA[col].dtypes == "O"]
for col in cat_cols:
    cat_summary(ALGEBRA, col, plot=False)

"""• En este código podemos observar en términos porcentuales como es la distribución de las categorías, y de acuerdo con estos porcentajes confirmamos que la dominancia de la categoría 'no' en la variable monitores es significativa. Por lo cuál es una variable que se analizará con mayor cuidado posteriormente, esto con el fin de tomar la decisión si se descarta o no.

• Por otra parte, las variables madre_tab y padre_tab tienen categorías que se pueden reagrupar, porque respecto a las demás categorías presentar algunos porcentajes bajos.
...
"""
Las estadísticas descriptivas de este conjunto de datos, compuesto por 651 registros, revelan que la edad promedio se sitúa en torno a los 16.75 años, con una moderada dispersión de alrededor de 1.22. Las edades varían desde un mínimo de 15 años hasta un máximo de 22 años. En lo que respecta a las calificaciones, las puntuaciones promedio en las tres notas se encuentran alrededor de 11.38, 11.57 y 11.90, respectivamente. Estos datos estadísticos ofrecen una visión general de la distribución y la variabilidad de las edades y las puntuaciones de notas en el conjunto de datos."""
...
num_cols = [col for col in ALGEBRA.columns if ALGEBRA[col].dtypes != "O"]
for col in num_cols:
    num_summary(ALGEBRA, col, plot=False)

"""La edad promedio de los estudiantes es de aproximadamente 16.75 años, con un rango de 15 a 22 años. La educación de los padres se sitúa en promedio alrededor de 2.5, indicando un nivel educativo moderado en general. Las calificaciones de los exámenes oscilan entre 1 y 4 en promedio, con una media de 1.57. La mayoría de los estudiantes tienen cero faltas, y la dinámica familiar varía de 1 a 5. La salud y las ausencias tienen una dispersión significativa. En cuanto a las notas de las evaluaciones, se encuentran alrededor de 11.38, 11.57 y 11.90 para nota_01, nota_02 y nota_03, respectivamente. Estos datos resumidos proporcionan una visión general de las características de los estudiantes en el conjunto de datos.
...
ALGEBRA.describe(include='object')

fig = make_subplots(rows=1, cols=2)

fig.add_trace(
    go.Histogram(x=ALGEBRA['nota_03'], name='Histograma de la nota 3', marker_color='coral'),
    row=1, col=1
)

fig.add_trace(
    go.Box(y=ALGEBRA['nota_03'], name='Boxplot de la nota 3', marker_color='coral'),
    row=1, col=2
)
...
"""La variable respuesta tiene mucha variabilidad en los datos, un posible sesgo a la izquierda y hay presencia de atípicos"""

# Calcular el rango intercuartílico (IQR)
Q1 = ALGEBRA['nota_03'].quantile(0.25)
Q3 = ALGEBRA['nota_03'].quantile(0.75)
IQR = Q3 - Q1
...
valores_atipicos = ALGEBRA[(ALGEBRA['nota_03'] < lower_bound) | (ALGEBRA['nota_03'] > upper_bound)]['nota_03']

# Obtener la cantidad de valores atípicos
print(f"Cantidad de valores atípicos: {len(valores_atipicos)}")

"""Se identifican 16 atípicos en esta variable
...
ALGEBRA['departamento'] = ALGEBRA['departamento'].replace('MS', 'IS')
ALGEBRA['departamento'].value_counts()

# crear gráfica
fig = px.pie(ALGEBRA, names='departamento', title ='<b>Departamento<b>')
...
"""Se observa que ambas categorias se encuentran balanceadas y no parece que vaya afectar negativamente el modelo.

2. *Sexo*
"""

# hacer la gráfica
fig = px.pie(ALGEBRA, names = 'sexo', title = '<b>Sexo<b>', hole = .5)
...
"""Se observa que ambas categorías se encuentran balanceadas y no parece que vaya afectar negativamente el modelo.
...
fig.add_trace(
    go.Histogram(x=ALGEBRA['edad'], name='Histograma de la edad', marker_color=px.colors.qualitative.Pastel1),
    row=1, col=1
)

fig.add_trace(
    go.Box(y=ALGEBRA['edad'], name='Boxplot de la edad', marker_color='purple'),
    row=1, col=2
)
...
"""De acuerdo con este primer resultado podemos observar que las edades de 20, 21 y 22 años tienen muy pocos registros, lo cual es imperceptible para el modelo. Sin embargo, para tratar este comportamiento vamos a reagrupar estos datos con la edad 19.

#Agrupación edades
"""

# Crear una copia del DataFrame para evitar modificar el original
ALGEBRA_nuevo = ALGEBRA.copy()

# Utilizar replace para modificar los valores en la nueva columna
ALGEBRA_nuevo['edad_agrup'] = ALGEBRA_nuevo['edad'].replace({22: 19, 21: 19, 20: 19})

# Ahora, ALGEBRA_nuevo contiene la variable 'edad_agrup' con la agrupación realizada

fig = make_subplots(rows=1, cols=2)

fig.add_trace(
    go.Histogram(x=ALGEBRA_nuevo['edad_agrup'], name='Histograma de la edad', marker_color=px.colors.qualitative.Pastel1),
    row=1, col=1
)

fig.add_trace(
    go.Box(y=ALGEBRA_nuevo['edad_agrup'], name='Boxplot de la edad', marker_color='#d62728'),
    row=1, col=2
)
...
"""Se observa la gráfica despues de realizar la agrupación de dichas edades y se nota un comportamiento más balanceado entre las categorías

*4*. *Dirección*
"""

# crear gráfica
fig = px.pie(ALGEBRA, names='direccion', title ='<b>Dirección<b>')
...
# crear gráfica
fig = px.pie(ALGEBRA, names='t_familia', title ='<b>Tamaño de la familia<b>')
...
# hacer la gráfica
fig = px.pie(ALGEBRA, names = 'padres_status', title = '<b>Estado de convivencia de los padres<b>', hole = .5)
...
df1 = ALGEBRA.groupby('madre_edu')['count'].sum().reset_index()
df2 = ALGEBRA.groupby('padre_edu')['count'].sum().reset_index()
...
"""Se observa un comportamiento similar de ambas variables, donde la categoría cero no tiene datos significativos, se decide reagrupar y quedaría de la siguiente manera: agrupamos la 0 y la 1, la 2 y la 3, la 4 se deja igual.
...
ALGEBRA_nuevo['madre_edu_agrup'] = ALGEBRA_nuevo['madre_edu'].replace({0: 1, 3: 2, 4: 3})

# Hacer lo mismo para la variable padre_edu
ALGEBRA_nuevo['padre_edu_agrup'] = ALGEBRA_nuevo['padre_edu'].replace({0: 1, 3: 2, 4: 3})

ALGEBRA_nuevo['count'] = 1

df1 = ALGEBRA_nuevo.groupby('madre_edu_agrup')['count'].sum().reset_index()
df2 = ALGEBRA_nuevo.groupby('padre_edu_agrup')['count'].sum().reset_index()
...
"""Se observa nuevamente la gráfico con la agrupación realizada y se nota un comportamiento más balanceado entre las categorías

9. *Trabajo de la madre*
10. *Trabajo del padre*
"""

df1 = ALGEBRA.groupby('madre_trab')['count'].sum().reset_index()
df2 = ALGEBRA.groupby('padre_trab')['count'].sum().reset_index()
...
fig.update_layout(
    ...
    yaxis_title='Cantidad',
    title_text='<b>Trabajo de los padres<b>',
    title_x=0.5,
    template='simple_white'
)
...
ALGEBRA_nuevo['madre_trab_agrup'] = ALGEBRA_nuevo['madre_trab'].replace({'salud': 'otro'})
ALGEBRA_nuevo['padre_trab_agrup'] = ALGEBRA_nuevo['padre_trab'].replace({'salud': 'otro'})

df1 = ALGEBRA_nuevo.groupby('madre_trab_agrup')['count'].sum().reset_index()
df2 = ALGEBRA_nuevo.groupby('padre_trab_agrup')['count'].sum().reset_index()

# Crear subplots
fig = make_subplots(rows=1, cols=2)

# Crear y agregar trazas de barras a los subplots
bar_trace1 = go.Bar(x=df1['madre_trab_agrup'], y=df1['count'], name='Madre',marker_color='burlywood')
bar_trace2 = go.Bar(x=df2['padre_trab_agrup'], y=df2['count'], name='Padre',marker_color='lightblue')
...
"""Se observa nuevamente la gráfica con la agrupación realizada y se nota que las categorías se encuentran más balanceadas

11. *Razón*
"""

# hacer la gráfica
fig = px.pie(ALGEBRA, names = 'razon', title = '<b>Razón por la cual eligió la carrera<b>', hole = .5)
...
# crear gráfica
fig = px.pie(ALGEBRA, names='guardian', title ='<b>Tutor del estudiante<b>')
...
fig = px.histogram(ALGEBRA, x='t_examen', title='<b>Tiempo que se demoró en el primer examen<b>')
...
# Crear gráfica de barras
fig = px.histogram(ALGEBRA, x='t_estudio', title='<b>Tiempo semanal que dedicó a estudiar<b>')
...
df_sum = ALGEBRA.groupby('faltas')['count'].sum().reset_index()
...
ALGEBRA_nuevo['faltas_agrup'] = ALGEBRA_nuevo['faltas'].apply(lambda x: 0 if x == 0 else 1)

# Calcular la suma de 'count' por grupo.
df_sum = ALGEBRA_nuevo.groupby('faltas_agrup')['count'].sum().reset_index()
...
# hacer la gráfica
fig = px.pie(ALGEBRA, names = 'soporte_edu_extra', title = '<b>¿Tiene el estudiante soporte educativo extra?<b>', hole = .5)
...
# hacer la gráfica
fig = px.pie(ALGEBRA, names = 'soporte_edu_fam', title = '<b>¿Tiene el estudiante soporte educativo familiar?<b>', hole = .5)
...
# hacer la gráfica
fig = px.pie(ALGEBRA, names = 'monitores', title = '<b>¿Tiene el estudiante clases extras pagadas por tutores privados?<b>'
)
...
"""la categoría "no" tiene el 94% de los datos, se esperará a observarla detalladamente en el análisis bivariado para ver su comportamiento y relevancia dentro del modelo

19. *Actividades extras*
"""

# hacer la gráfica
fig = px.pie(ALGEBRA, names = 'actividades_extra', title = '<b>¿Tiene el estudiante actividades extraescolares?<b>', hole = .5)
...
df_sum = ALGEBRA.groupby('preuniversitario')['count'].sum().reset_index()

# Crear gráfica de barras
fig = px.bar(df_sum, x='preuniversitario', y='count', title='<b>¿El estudiante asistió al preuniversitario?<b>')
...
# hacer la gráfica
fig = px.pie(ALGEBRA, names = 'postgrado', title = '<b>¿El estudiante quiere hacer postgrado?<b>', hole = .5)
...
"""En esta variable se nota que las categoría están desbalanceadas, con un 89% contra un 10%, esto no sería significativo para el modelo ya que sería como agregarle una constante, pero igual se va a ver en el análisis bivariado y se tomará una decisión más adelante.

22. *Internet*
"""

# Cambiar el nombre de las categorias a español
ALGEBRA['internet'] = ALGEBRA['internet'].replace('yes', 'si')

# hacer la gráfica
fig = px.pie(ALGEBRA, names = 'internet', title = '<b>¿El estudiante tiene acceso a Internet en casa?<b>')
...
"""La categoría "si" se ve un poco cargada ya que contiene casi el 77% del total de estudiantes, igualmente se analizará esta variable más adelante.

23. *Relación sentimental*
"""

# Cambiar el nombre de las categorias a español
ALGEBRA['relacion_sen'] = ALGEBRA['relacion_sen'].replace('yes', 'si')

# hacer la gráfica
fig = px.pie(ALGEBRA, names = 'relacion_sen', title = '<b>¿El estudiante tiene relación sentimental?<b>', hole = .5)
...
"""El 63.3% de los estudiantes tienen una relación sentimental, las categorías no parecen estar desbalanceadas, más adelante se analizará su relación con la variable independiente y se considerará su importancia dentro del modelo.

24. *Relación familiar*
"""

df = ALGEBRA.groupby('relacion_fam')['count'].sum().reset_index()
...
"""En esta variable se observa que la mayoría califican en 4 la relación con su familia y vemos que muy pocos tienen puntajes entre 1 y 2. Se decide agrupar las categorías 1 y 2, la 3 se dejará igual y agrupar la 4 con la 5, con esta nueva clasificación se espera ver más fácil los que consideran tener una mala relación, una regular y una buena."""

ALGEBRA_nuevo['relacion_fam_agrup'] = ALGEBRA['relacion_fam']
ALGEBRA_nuevo['relacion_fam_agrup'] = ALGEBRA_nuevo['relacion_fam_agrup'].apply(lambda x: x + 1 if x in [1, 4] else x)
ALGEBRA_nuevo['relacion_fam_agrup'] = ALGEBRA_nuevo['relacion_fam_agrup'].replace({2: 1, 3: 2, 5: 3})

df = ALGEBRA_nuevo.groupby('relacion_fam_agrup')['count'].sum().reset_index()
...
"""Se vuelve a observar la gráfica y se ve un comportamiento un poco más balanceado

25. *Tiempo libre*
"""

df = ALGEBRA.groupby('tiempo_libre')['count'].sum().reset_index()
...
df = ALGEBRA.groupby('salir_amigos')['count'].sum().reset_index()
...
df = ALGEBRA.groupby('cons_alcohol_sem')['count'].sum().reset_index()
...
df = ALGEBRA.groupby('cons_alcohol_finde')['count'].sum().reset_index()
...
df = ALGEBRA.groupby('salud')['count'].sum().reset_index()
...
fig.add_trace(
    go.Histogram(x=ALGEBRA['ausencias'], name='Histograma de las ausencias', marker_color='turquoise'),
    row=1, col=1
)

fig.add_trace(
    go.Box(y=ALGEBRA['ausencias'], name='Boxplot de las ausencias', marker_color='turquoise'),
    row=1, col=2
)
...
def percentiles(ALGEBRA, lim_inf, lim_sup):
  lim_inf = lim_inf
  lim_sup = lim_sup
  outliers = ALGEBRA[(ALGEBRA < ALGEBRA.quantile(lim_inf)) |(ALGEBRA > ALGEBRA.quantile(lim_sup))]
  return outliers

ausencias_outlier_index = percentiles(ALGEBRA['ausencias'], 0.01, 0.99).index.tolist()
ausencias_outlier_values = percentiles(ALGEBRA['ausencias'], 0.01, 0.99).values.tolist()
...
"""La variable ausencia tiene 6 atípicos, los cuales representa casi un 1% de los datos. De acuerdo con esto, sabemos que un 1% de datos atípicos es una cifra poco significativa y que no afectaría drásticamente al modelo. Sin embargo, por cuestiones de limpieza de los datos consideramos que estos registros sean eliminados."""

# Eliminar atipicos
ALGEBRA = ALGEBRA.drop([150, 155, 197, 212, 256, 325])

fig = make_subplots(rows=1, cols=2)

fig.add_trace(
    go.Histogram(x=ALGEBRA['ausencias'], name='Histograma de las ausencias', marker_color='lightgreen'),
    row=1, col=1
)
...
"""Se observa nuevamente la gráfica y se nota que mejoró en cuanto a datos atípicos
...
fig.add_trace(
    go.Histogram(x=ALGEBRA['nota_01'], name='Histograma de la nota 1', marker_color='coral'),
    row=1, col=1
)

fig.add_trace(
    go.Box(y=ALGEBRA['nota_01'], name='Boxplot de la nota 1', marker_color='coral'),
    row=1, col=2
)
...
"""Para esta variable se nota un comportamiento normal, con poca presencia de atípicos, la proporción de atípicos es muy baja y se considera que no afectarán al modelo, además eliminarlos puede extraer información que puede ser útil en el modelo.
...
fig.add_trace(
    go.Histogram(x=ALGEBRA['nota_02'], name='Histograma de la nota 2', marker_color='lightblue'),
    row=1, col=1
)

fig.add_trace(
    go.Box(y=ALGEBRA['nota_02'], name='Boxplot de la nota 2', marker_color='lightgreen'),
    row=1, col=2
)
...
# crear dataset
base = ALGEBRA.groupby(['departamento'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)

# crear gráfica
fig = px.bar(base, x='departamento', y='nota_03', barmode ='group', title ='<b>Promedio Nota 3 según la carrera<b>')
...
"""No se observa un cambio significativo de la nota 3 con respecto al sexo de los estudiantes. Por lo tanto, no será considerada dentro del modelo.

Nota 3 vs Edad
"""

# crear base
base = ALGEBRA.groupby(['edad'])[['nota_03']].mean().reset_index().sort_values('edad', ascending = False)
...
"""En este gráfico se puede observar que hay una tendencia que a mayor edad menor nota, con un pico en los 19 y 20 años. Esta variable será considerada en el modelo

Dirección vs Nota 3
"""

# crear dataset
base = ALGEBRA.groupby(['direccion'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)

# crear gráfica
fig = px.bar(base, x='direccion', y='nota_03', barmode ='group', title ='<b>promedio Nota 3 segun su dirección<b>')
...
# crear dataset
base = ALGEBRA.groupby(['t_familia'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)

# crear gráfica
fig = px.bar(base, x='t_familia', y='nota_03', barmode ='group', title ='<b>promedio Nota 3 según el tamaño de la familia<b>')
...
nota_por_status = ALGEBRA.groupby('padres_status')['nota_03'].mean().sort_values(ascending=True)
status_juntos = ALGEBRA.loc[ALGEBRA['padres_status']=='J']
status_separados = ALGEBRA.loc[ALGEBRA['padres_status']=='S']
...
fig.add_trace(
    go.Box(y=status_juntos['nota_03'], name='J', marker_color='coral'),
    row=1, col=2
)
...
fig.update_layout(title_text="Promedio Nota 3 segun el status de los padres", template='simple_white')
fig.show()
...
base1 = ALGEBRA.groupby(['madre_edu'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending=True)
...
# crear dataset para el primer gráfico
base1 = ALGEBRA.groupby(['madre_trab'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending=True)
...
# crear dataset para el segundo gráfico
base2 = ALGEBRA.groupby(['padre_trab'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending=True)
...
nota_por_razon = ALGEBRA.groupby('razon')['nota_03'].mean().sort_values(ascending=True)
razon_recomendacion = ALGEBRA.loc[ALGEBRA['razon']=='recomendacion']
razon_reputacion = ALGEBRA.loc[ALGEBRA['razon']=='reputacion']
razon_habilidad = ALGEBRA.loc[ALGEBRA['razon']=='habilidad']
razon_otro = ALGEBRA.loc[ALGEBRA['razon']=='otro']
...
"""La razón por la cual se escogió la carrera no parece afectar mucho los resultados en la nota 3, por lo que se decide que no va a aportar información importante en el modelo y no será incluida en este.

Nota 3 vs guardián o tutor del estudiante
"""

# crear dataset
base = ALGEBRA.groupby(['guardian'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)

# crear gráfica
fig = px.bar(base, x='guardian', y='nota_03', barmode ='group', title ='<b>Promedio Nota 3 según el tutor del estudiante<b>')

# cambiar el color
fig.update_traces(marker_color='#AA88EE')

# agregar detalles a la gráfica
fig.update_layout(
    xaxis_title = 'guardián',
    yaxis_title = 'nota_3',
    template = 'simple_white',
    title_x = 0.5)
...
base = ALGEBRA.groupby(['t_examen'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending=True)
...
base_line = ALGEBRA.groupby(['t_examen'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending=False)
...
# crear base
base = ALGEBRA.groupby(['t_estudio'])[['nota_03']].mean().reset_index().sort_values('t_estudio', ascending = False)
...
# definir gráfica
fig.update_layout(
    title ='<b>Promedio Nota 3 segun el tiempo de estudio<b>',
...
# crear dataset
base = ALGEBRA.groupby(['faltas'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)
...
# crear dataset
base = ALGEBRA.groupby(['soporte_edu_extra'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)

# crear gráfica
fig = px.bar(base, x='soporte_edu_extra', y='nota_03', barmode ='group', title ='<b>Promedio Nota 3 segun el apoyo educativo adicional<b>')
...
# crear dataset
base = ALGEBRA.groupby(['soporte_edu_fam'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)

# crear gráfica
fig = px.bar(base, x='soporte_edu_fam', y='nota_03', barmode ='group', title ='<b>Promedio Nota 3 segun el apoyo educativo familiar<b>')
...
# crear dataset
base = ALGEBRA.groupby(['monitores'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)
...
# crear dataset
base = ALGEBRA.groupby(['actividades_extra'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)
...
# crear dataset
base = ALGEBRA.groupby(['preuniversitario'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)
...
# crear dataset
base = ALGEBRA.groupby(['postgrado'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)
...
nota_internet = ALGEBRA.groupby('internet')['nota_03'].mean().sort_values(ascending=True)
internet_si = ALGEBRA.loc[ALGEBRA['internet']=='si']
internet_no = ALGEBRA.loc[ALGEBRA['internet']=='no']
...
Nota 3 vs relación sentimental
"""

# crear dataset
base = ALGEBRA.groupby(['relacion_sen'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)

# crear gráfica
fig = px.bar(base, x='relacion_sen', y='nota_03', barmode ='group', title ='<b>Promedio Nota 3 vs relación sentimental<b>')
...
base1 = ALGEBRA.groupby(['relacion_fam'])[['nota_03']].mean().reset_index().sort_values('relacion_fam', ascending=False)

# Crear la base de datos para la segunda gráfica
base2 = ALGEBRA.groupby(['relacion_fam'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending=True)
...
# Crear la base de datos para la primera gráfica
base1 = ALGEBRA.groupby(['tiempo_libre'])[['nota_03']].mean().reset_index().sort_values('tiempo_libre', ascending=False)

# Crear la base de datos para la segunda gráfica
base2 = ALGEBRA.groupby(['tiempo_libre'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending=True)
...
# Crear la base de datos para la primera gráfica
base1 = ALGEBRA.groupby(['salir_amigos'])[['nota_03']].mean().reset_index().sort_values('salir_amigos', ascending=False)

# Crear la base de datos para la segunda gráfica
base2 = ALGEBRA.groupby(['salir_amigos'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending=True)
...
# Crear la base de datos para la primera gráfica
base1 = ALGEBRA.groupby(['cons_alcohol_sem'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending=True)

# Crear la base de datos para la segunda gráfica
base2 = ALGEBRA.groupby(['cons_alcohol_finde'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending=True)
...
# Crear la figura con dos subplots uno al lado del otro
fig = make_subplots(rows=1, cols=2, subplot_titles=("Nota 3 vs consumo alcohol jornada laboral", "Nota 3 vs consumo alcohol finde"))
fig.add_trace(fig1.data[0], row=1, col=1)
fig.add_trace(fig2.data[0], row=1, col=2)
...
"""Notamos un comportamiento un poco similar entre ambas variables y puede ser interesante su comportamiento en el modelo, se decide incluirlas en este

Nota 3 vs salud
"""

# crear dataset
base = ALGEBRA.groupby(['salud'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)
...
# crear dataset
base = ALGEBRA.groupby(['ausencias'])[['nota_03']].mean().reset_index().sort_values('nota_03', ascending = True)
...
"""Se nota mucha variabilidad en los datos, se observa que a más ausencias del estudiante realmente no se ve muy afectada la nota, por lo que esta variable no será considerada en el modelo.

Nota 3 vs Nota 1 y Nota 2
"""

# Crear la base de datos para la primera gráfica
base = ALGEBRA.groupby(['nota_01'])[['nota_03']].mean().reset_index().sort_values('nota_01', ascending=False)

# Crear la base de datos para la segunda gráfica
base2 = ALGEBRA.groupby(['nota_02'])[['nota_03']].mean().reset_index().sort_values('nota_02', ascending=False)
...
"""A mayor resultado en los examenes 1 y 2, mayor la nota 3, se observa una relación directamente proporcional aunque con un pico en el 5 en la nota 1
...
variables_numericas = ALGEBRA.select_dtypes(include=['number'])
...
"""Se han identificado cinco correlaciones significativas, entre las que se destaca la similitud entre las variables de educación de la madre y del padre, lo que resalta la importancia de abordar la multicolinealidad en el análisis. Además, se ha observado la relación entre salir con amigos y el consumo de alcohol durante el fin de semana, lo que podría tener implicaciones importantes en el análisis. Asimismo, se ha señalado la correlación entre el consumo de alcohol en la jornada laboral y en los fines de semana, lo que podría ser crucial para comprender los patrones generales de consumo. Así mismo, se encontro fuerte relación entre las variables tiempo libre y salir con amigos y las variables lo de faltas y la edad del estudiante. En conjunto, esta evaluación demuestra un enfoque lógico y orientado hacia la acción, con la intención de profundizar en estas correlaciones en el modelo para obtener insights valiosos de los datos.

###Para variables categórica con categórica

### Prueba Chi-Cuadrado entre variables categóricas
"""

# Filtra las columnas que son de tipo 'object' (categóricas).
categorical_columns = ALGEBRA.select_dtypes(include=['object'])
...
corralg = ALGEBRA.corr()
corralg

# @title **Relaciones entre Variables**
sns.pairplot(ALGEBRA)
plt.show()

"""## Selección de variables"""

# Agregamos la columna reprobado para la nota 3, para volverla binaria
ALGEBRA['nota_3'] = ALGEBRA['nota_03'].apply(lambda x: 0 if x < 12 else 1)

y_target = ALGEBRA['nota_3']  #Variable objetivo que queremos predecir.
X_features = ALGEBRA.drop(['sexo','direccion', 't_familia', 'padres_status', 'madre_trab', 'padre_trab', 'razon', 'guardian', 't_examen', 'soporte_edu_extra', 'soporte_edu_fam',
'monitores', 'actividades_extra', 'preuniversitario','postgrado', 'relacion_fam', 'salud', 'ausencias','nota_03', 'count', 'nota_3'], axis=1, inplace=False)
...
# crear dataset
base = ALGEBRA.groupby(['nota_3'])[['count']].count().reset_index().sort_values('count', ascending = False)
...
"""Esto nos indica un buen rendimiento del modelo: Tiene una precisión del 92%, con una métrica sólida<br>
La precisión es del 90%, las predicciones positivas son realmente positivas<br>
El modelo captura el 94% de los estudiantes que realmente pasaron <br>
Y el modelo tiene un buen poder discriminatorio entre clases positivas y negativas. roc_auc del 92%
"""
...
"""#Probando la regresión lineal para Álgebra"""

from sklearn.preprocessing import LabelEncoder

label_encoder = LabelEncoder()
ALGEBRA['nota_3_encoded'] = label_encoder.fit_transform(ALGEBRA['nota_3']) #Codificamos la nota_3

print(ALGEBRA.head()) #Revisamos que haya quedado ok
...
print(f'MSE: {mse}') #Calculamos el error cuadrático medio, entre más bajo, más ajustado el modelo.
print(f'R^2: {r2}') #Calculamos el coeficiente de determinación, es la proporción de la varianza total en la variable objetivo.

"""El R^2 explica aproximadamente el 49.4 de la variabilidad de la nota_3"""
...
#Revisamos el error cuadrático medio
from sklearn.model_selection import cross_val_score

scores = cross_val_score(model, X_features, y_target, cv=5, scoring='neg_mean_squared_error')
rmse_scores = np.sqrt(-scores)

print('RMSE promedio:', rmse_scores.mean())

"""RMSE promedio de 0.33216467473950745 las predicciones del modelo de regresión están a una distancia de aproximadamente 0.33216467473950745 unidades (en las mismas unidades que la variable objetivo) de los valores reales."""